"""
Experiment: sensing MI vs number of sensing targets Ks.

Sweeps Ks in [2, 4, 6, 8, 10] with 10 Monte Carlo trials each.
Array: 128x8, Pt=10 dBm, N=Kc+Ks (dynamic), Kc=2, Ke=2.
Both fully_connected and subarray structures.

Baselines
---------
  noholo : a=1 (fixed), W optimised via SP3' SDP
  allon  : a=1, isotropic W scaled to Pt
  rand   : a ~ U[0,1], isotropic W scaled to Pt

Usage:
    python main.py ks_sensing
    python experiments/exp_ks_sensing.py
"""
import copy
import json
import sys
...
def main():
    ...
    base_cfg = SystemConfig(
        Mt_h=128, Mt_v=8, Mr_h=128, Mr_v=8,
        Kc=2, Ke=2, L=128, seed=SEED,
        Pt_dBm=10.0,
        # N and Ks are set per sweep point below
    )
    alg_cfg = AlgorithmConfig(outer_iters=10, inner_iters=100, pgd_step_a=5e-4)

    out = {"label": "exp_ks_sensing",
           "Ks": Ks_list, "n_trials": n_trials,
           "fully_connected": {m: {"mean": [], "std": []}
                                for m in ["soop2", "noholo", "allon", "rand"]},
           "subarray":        {m: {"mean": [], "std": []}
                                for m in ["soop2", "noholo", "allon", "rand"]}}

    print(f"[exp_ks_sensing] Mt=128x8  Pt=10 dBm  Kc=2  Ke=2  "
          f"{n_trials} trials  outer={alg_cfg.outer_iters}")
    print("  N = Kc + Ks (dynamic)")
    print("-" * 65)

    Mt = base_cfg.Mt_h * base_cfg.Mt_v   # 1024
    ...
    for struct in structs:
        ...
        for ks in Ks_list:
            N = base_cfg.Kc + ks
            # subarray requires N | Mt; skip incompatible points
            if struct == "subarray" and Mt % N != 0:
                print(f"    Ks={ks}  N={N}  SKIP (N∤Mt={Mt} for subarray)")
                for m in ["soop2", "noholo", "allon", "rand"]:
                    out[struct][m]["mean"].append(float("nan"))
                    out[struct][m]["std"].append(float("nan"))
                continue
    ...
    path = RESULTS_DIR / f"exp_ks_sensing_{ts}.json"
    with open(path, "w") as f:
        json.dump(_to_jsonable(out), f, indent=2)
    print(f"\n[exp_ks_sensing] saved -> {path}")
